...
import numpy as np
from gpytorch.kernels import ScaleKernel, RBFKernel, PeriodicKernel,LinearKernel,SpectralDeltaKernel,RQKernel
...
class ExactGPModel(gpytorch.models.ExactGP):
    def __init__(self, train_x, train_y, likelihood):
        ...
        self.covar_module = ScaleKernel(LinearKernel())+ScaleKernel(RBFKernel(lengthscale_constraint=gpytorch.constraints.Interval(55,130),outputscale_constraint = gpytorch.constraints.Interval(4000,6000))+ScaleKernel(RBFKernel(lengthscale_constraint=gpytorch.constraints.Interval(200,400),outputscale_constraint =gpytorch.constraints.Interval(150,200))*PeriodicKernel(period_length_constraint=gpytorch.constraints.Interval(0,12), lengthscale_constraint=gpytorch.constraints.Interval(2,4)))+ScaleKernel(RQKernel(alpha_constraint = gpytorch.constraints.Interval(10,20)))+ScaleKernel(RQKernel(alpha_constraint = gpytorch.constraints.Interval(20,40))))
